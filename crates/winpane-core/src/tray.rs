...
pub(crate) unsafe fn create_hicon_from_rgba(
    ...
    // SAFETY: Win32 GDI calls with valid HDC/bitmap handles.
    unsafe {
...
pub(crate) unsafe fn create_tray_icon(
    ...
    // SAFETY: Shell_NotifyIconW called with valid HWND and populated NOTIFYICONDATAW.
    unsafe {
...
pub(crate) unsafe fn update_tray_icon(
    hwnd: HWND,
    icon_id: u32,
    hicon: HICON,
) -> std::result::Result<(), Error> {
...
pub(crate) unsafe fn destroy_tray_icon(hwnd: HWND, icon_id: u32, hicon: HICON) {
    // SAFETY: DestroyIcon/Shell_NotifyIconW with handles created by the caller.
    unsafe {
        let nid = NOTIFYICONDATAW {
            cbSize: size_of::<NOTIFYICONDATAW>() as u32,
            hWnd: hwnd,
...
pub(crate) unsafe fn show_tray_context_menu(hwnd: HWND, items: &[MenuItem]) -> u32 {
    ...
    // SAFETY: Win32 menu APIs with valid HWND and HMENU.
    unsafe {
